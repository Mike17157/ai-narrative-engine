...
just the krea2 pipeline + LoRA customization. Kept here (not deleted) in case
any of it is wanted again later — RunPod serverless, the complex-workflow
graph editor/importer, GPU-placement auto-routing, and the non-krea2 model
families (anima, flux, wan, illustrious).
...
- `loom/comfy/family.py`, `scan.py`, `librarian.py` — general model-file
  taxonomy/organizer, still used for LoRA↔checkpoint compatibility matching.
- `loom/comfy/stack.py`, `configs/image_presets.json`,
  `loom/server/services/image_presets.py`, `loom/server/routers/image_presets.py`,
  the `ImagePresetPicker`/`library/image-presets` frontend — LoRA customization,
  which never depended on any of the above.
