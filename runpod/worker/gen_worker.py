...
NODE_REPO: dict[str, tuple[str, str]] = {
    ...
    # ComfyUI-Impact-Subpack — must be cloned INSIDE Impact-Pack (see NESTED_REPOS below)
...
# Some packs must live INSIDE another pack's directory, not at the top level of
# custom_nodes/.  Impact-Pack looks for its subpack at
# ComfyUI-Impact-Pack/impact_subpack/ — cloning it as a sibling means
# UltralyticsDetectorProvider never registers and every workflow using it gets a 400.
# key = repo url, value = (parent_clone_dir, subdir_name)
NESTED_REPOS: dict[str, tuple[str, str]] = {
...
def main() -> None:
    ...
    # ---- Dockerfile ----------------------------------------------------------
    top_level_repos = [url for url in sorted(repos) if url not in NESTED_REPOS]
    clone_top = " && \\\n    ".join(f"git clone --depth 1 {url}" for url in top_level_repos)
    # Nested repos go inside a parent dir (e.g. Impact-Subpack → ComfyUI-Impact-Pack/impact_subpack/)
    clone_nested_cmds = []
    for url in sorted(repos):
        if url in NESTED_REPOS:
            parent, subdir = NESTED_REPOS[url]
            clone_nested_cmds.append(f"git clone --depth 1 {url} {parent}/{subdir}")
    clone_nested = (" && \\\n    ".join(clone_nested_cmds)) if clone_nested_cmds else ""
    clone_block = clone_top
    if clone_nested:
        clone_block += " && \\\n    " + clone_nested
...
# The base image checks out a tagged release in detached HEAD, so git pull won't work;
# fetch + checkout master gives us the full history and installs any new deps.
RUN cd /comfyui && git fetch origin && git checkout origin/master && pip install --no-cache-dir -r requirements.txt

WORKDIR /comfyui/custom_nodes
# NOTE: Impact-Subpack is cloned INSIDE ComfyUI-Impact-Pack/ (not at the top level)
# because Impact-Pack looks for its subpack at ComfyUI-Impact-Pack/impact_subpack/.
# Cloning it as a sibling means UltralyticsDetectorProvider never registers → 400.
RUN {clone_block}
