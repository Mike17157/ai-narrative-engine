...
# Layer taxonomy — ordered as the tabs flow. `mutators` declares which surfaces may
# write the layer (author = the tab UIs + plot-dialogue functions; play = the runtime
# via consolidation). Advisory for now; enforcement lives at the patch chokepoint.
LAYERS: tuple[dict, ...] = (
    {"id": "overview",      "source": "overview",      "mutators": ("author",)},
    {"id": "plot",          "source": "plot",          "mutators": ("author", "play")},
    {"id": "relationships", "source": "relationships", "mutators": ("author", "play")},
    {"id": "map",           "source": "map",           "mutators": ("author", "play")},
    {"id": "cast",          "source": "cast",          "mutators": ("author", "play")},
)
...
LAYER_FIELDS: dict[str, tuple[str, ...]] = {
    ...
    "map":           ("locations", "places", "start", "connections"),
}
...
def build_card(story: dict, manifests: dict[str, dict] | None = None,
    ...
    contents = {"overview": (overview, overview_todo), "plot": (plot, plot_todo),
                "relationships": (relationships, rel_todo), "map": ({"locations": map_locs}, map_todo),
