...
_UNIFIED_OUTFIT_SYSTEM = (
    ...
    "  • A single pose, given as LITERAL body position only — where the weight sits, what the hands "
    "do, the tilt of the head (e.g. 'leaning back, arms loosely crossed', 'hands clasped behind the "
    "back, heels together, chin slightly down'). Describe the POSITION, NEVER what it means or "
    "implies. FORBIDDEN: 'as if', 'as though', 'reads as', 'masking', 'hinting', 'betraying', "
    "'suggesting', 'beneath', 'the ___ of someone who'. No inner states, motives, or backstory.\n\n"
    "Style rules:\n"
...
_NL_BASE_SYSTEM = (
    ...
    "  (B) WRITE the `prompt` field as a STRUCTURED, precise natural-language appearance"
    ...
    "(the workflow carries its own quality/style tags), so it must be concrete, exact and complete — every clause names a VISIBLE feature.\n\n"
    ...
    "same default pretty).\n"
    "  2. HAIR — colour, length, the primary style AND how it falls, plus a telling detail; how it "
    "frames the face. State the STYLE as a visible fact (neatly controlled, loose and unruly, "
    "formally arranged) — the style itself, never what it says about them.\n"
    "  3. FACE & EYES — the shape of the face, the eyes (colour + shape), the brows. Give the EYE "
    "SHAPE as the plain visible fact (soft rounded tareme, sharp upturned tsurime) and the resting "
    "set of the eyes and mouth as a position (level, relaxed, a slight downturn or upward tug) — the "
    "shape, not what it means. Eyes open and alert. Note any distinguishing mark (a mole, freckles, "
    "a scar, heterochromia, glasses, makeup).\n"
    ...
    "  6. PRESENCE — a plain standing pose (where the weight sits, what the hands do) and the resting "
    "gaze (most meet the viewer's eyes; some look aside). Position only — no 'air', 'aura', or mood.\n"
    ...
    "eyes, the expression, the pose and the build — they must COHERE. But personality shows ONLY as a VISIBLE feature stated as a BARE FACT — 'messy dark hair falling into his eyes', 'sharp almond eyes, slightly narrowed', 'weight on one hip, one hand at his belt'. Describe what is SEEN, NEVER what it means, implies, or where it came from. FORBIDDEN constructions: 'as if', 'as though', 'reads as', 'seems', 'beneath the ___', 'masking', 'hinting', 'suggesting', 'betraying', 'carrying the ___ of', 'the ___ of someone who', and any backstory clause ('forced to grow up too fast', 'lives by his wits'). The image model renders pixels, not psychology. Do not describe a shy character"
