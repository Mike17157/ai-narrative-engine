<script>
  ...
  // View a saved dataset and (re)caption it with a vision model via OpenRouter.
  let datasets = $state([]);
  let dataset = $state('');
  ...
  async function saveCaption(img) {
    await post('/lora/caption/edit', { dataset, file: img.file, caption: img.caption });
  }

  // Start a resident captioning job, then attach to its stream. The job runs on
  // the server, so it survives navigation and shows in the Activity panel.
...
{#if detail}
  <div class="grid" style="grid-template-columns: repeat(auto-fill, minmax({cardSize}px, 1fr)); --cap-h: {capH}px">
    {#each detail.images as img (img.file)}
      <figure class="card">
        <img src={`/api/lora/datasets/${encodeURIComponent(dataset)}/img/${img.file}`} alt={img.file} loading="lazy" />
        <textarea bind:value={img.caption} onblur={() => saveCaption(img)} placeholder="(no caption)"></textarea>
      </figure>
    {/each}
  </div>
...
{:else}
  <div class="hint" style="margin-top:18px">No dataset selected. Curate one under Generation → Save LoRA Set first.</div>
{/if}
