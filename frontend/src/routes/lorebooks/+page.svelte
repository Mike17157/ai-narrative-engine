<script>
  ...
  const CATEGORIES = ['world', 'story', 'rpg', 'character', 'craft', 'intimacy', 'guard'];
  ...
  async function saveMeta() {
    if (!detail) return;
    const { name, description, rating, category, enabled } = detail.book;
    const r = await patch('/lorebooks/' + encodeURIComponent(selId), { name, description, rating, category, enabled });
...
<div class="mgr">
  ...
  <section class="ed">
    ...
    {:else}
      ...
      <!-- Entries -->
      <div class="enthead">
        <h3>Entries <span class="cnt">{detail.entries.length}</span></h3>
