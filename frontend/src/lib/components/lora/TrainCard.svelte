<script>
  // Trains a LoRA from a saved dataset using kohya sd-scripts (SDXL/SD1.5), or
  // generates a config for the external Anima DiT trainer. The trainer venv
  ...
  // Re-fetch the dataset list after a new dataset lands — the host page can
  // call this once GenerateCard saves.
  export async function refresh() { datasets = (await get('/lora/datasets')).datasets || []; }
