<script>
  import { app, refreshActivity, refreshHealth, cancelJob } from '../app.svelte.js';
  ...
  {:else}
    {#each jobs as j (j.id)}
      <div class="job" class:run={j.status === 'running'}>
        ...
        {#if j.status === 'running'}
          <div class="bar" class:indet={pct(j) === null}><span style={pct(j) !== null ? `width:${pct(j)}%` : ''}></span></div>
        {/if}
        <div class="meta">
...
<style>
  .menu {
    position: absolute; top: 42px; right: 0; z-index: 60; width: 320px;
  ...
  .bar { height: 6px; border-radius: 999px; background: var(--elev); overflow: hidden; border: 1px solid var(--border); margin: 8px 0 6px; }
  .bar span { display: block; height: 100%; background: linear-gradient(90deg, var(--accent), #9a6dff); transition: width .3s; }
  .bar.indet span { width: 32%; animation: slide 1.1s ease-in-out infinite; }
