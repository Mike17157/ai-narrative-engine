<script>
  ...
  // data._onClick is injected by StoryGraph — call it so arc header opens the arc modal
...
<!-- pointer-events:none on the lane background prevents it swallowing clicks on chapters -->
<div class="lane" class:empty={data.empty} style={`width:${w}px;height:${h}px`}>
  <div class="lane-head" onclick={() => data._onClick?.(data)} style="pointer-events:auto;cursor:pointer;">
    <span class="lane-name">{data.name}</span>
    {#if data.dramatic_function}<span class="df">{data.dramatic_function}</span>{/if}
    {#if data.divergence_axis}<span class="axis">⑂ {data.divergence_axis}</span>{/if}
  </div>
  {#if data.empty}
    <div class="lane-empty">Not expanded yet — switch to List view to generate timelines</div>
  {/if}
</div>
...
<style>
  .lane {
    ...
    padding: 8px 12px;
    pointer-events: none; /* let clicks pass through to chapter cards beneath */
  }
  .lane-head:hover { color: var(--accent, #6d8cff); }
  .axis {
    font-size: 9.5px; font-weight: 600; padding: 1px 6px; border-radius: 999px;
    background: rgba(255,200,90,.1); border: 1px solid rgba(255,200,90,.25);
    color: rgba(255,200,90,.9); white-space: nowrap;
  }
